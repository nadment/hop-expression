...
/**
 * A common implementation of the {@link BaseTransformDialog} that creates many of the common UI
 * components.
 *
 * <p>TODO: Move to org.apache.hop.pipeline.dialog but for now there is a problem with class loader
 */
public abstract class AbstractTransformDialog<META extends ITransformMeta<?, ?>>
    extends BaseTransformDialog implements ITransformDialog {
  ...
  public AbstractTransformDialog(
      Shell parent, IVariables variables, Object transformMeta, PipelineMeta pipelineMeta, String name) {
    super(parent, variables, (META) transformMeta, pipelineMeta, name);
  ...
  protected final Control createContents(final Composite parent) {
    ...
    titleSeparator.setLayoutData(
        new FormDataBuilder().top(titleArea, LARGE_MARGIN).fullWidth().result());
    ...
    bottomSeparator.setLayoutData(
        new FormDataBuilder().bottom(buttonBar, -LARGE_MARGIN).fullWidth().result());
    ...
    area.setLayoutData(
        new FormDataBuilder()
            .top(titleSeparator, LARGE_MARGIN)
            .bottom(bottomSeparator, -LARGE_MARGIN)
            .fullWidth()
            .result());
    props.setLook(area);
   ...
   *
   * <p>The <code>Dialog</code> implementation of this framework method creates and returns a new
   * <code>Composite</code> with no margins and spacing. Subclasses should override.
   *
  ...
  protected void createButtonsForButtonBar(final Composite parent) {
    ...
    wOk.setLayoutData(
        new FormDataBuilder().bottom().right(wCancel, -ConstUi.SMALL_MARGIN).result());
    wOk.addListener(
        SWT.Selection,
        new Listener() {
          @Override
  ...
  /**
   * This method is called by Spoon when the user opens the settings dialog of the step. It opens
   * the dialog and returns only once the dialog has been closed by the user.
   *
   * <p>If the user confirms the dialog, the meta object (passed in the constructor) is updated to
   * reflect the new step settings. The changed flag of the meta object reflect whether the step
   * configuration was changed by the dialog.
   *
   * <p>If the user cancels the dialog, the meta object is not updated
   *
   * <p>The open() method returns the name of the step after the user has confirmed the dialog, or
  ...
  /**
   * Returns a point describing the minimum receiver's size. The x coordinate of the result is the
   * minimum width of the receiver. The y coordinate of the result is the minimum height of the
   * receiver.
   *
  ...
  /**
   * This helper method takes the information configured in the dialog controls and stores it into
   * the step configuration meta object
   */
  /**
   * Creates a new instance of {@link META} and populates it with provided data from the dialog.
   *
