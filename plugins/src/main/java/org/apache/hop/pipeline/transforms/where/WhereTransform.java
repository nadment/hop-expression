...
public class WhereTransform extends BaseTransform<WhereMeta,WhereData> {
	...
	public WhereTransform(TransformMeta transformMeta, WhereMeta meta, WhereData data, int copyNr, PipelineMeta pipelineMeta,
			Pipeline pipeline) {
	...
	public boolean processRow() throws HopException {
		...
		if (first) {
			...
			if (!Utils.isEmpty(streams.get(0).getTransformName())) {
				data.trueRowSet = findOutputRowSet(getTransformName(), getCopy(), streams.get(0).getTransformName(), 0);
				if (data.trueRowSet == null) {
					throw new HopException(BaseMessages.getString(PKG, "ExpressionFilter.Log.TargetTransformInvalid",
			...
			if (!Utils.isEmpty(streams.get(1).getTransformName())) {
				data.falseRowSet = findOutputRowSet(getTransformName(), getCopy(), streams.get(1).getTransformName(), 0);
