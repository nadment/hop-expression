...
Interval literals defines a constant elapsed time. You can use these interval literals in conditions and calculations that involve datetime expressions. 
...
Plurals of each unit are also supported.

Interval constants can also be expressed using the following formats: INTERVAL 'Y-M D H:M:S'
