...
public class ExpressionParser {
    ...
    private IExpression parseLogicalOr() throws ExpressionException {
        ...
        while (lexer.isThenNextAndNotEnd(Id.OR)) {
            expression = new Call(lexer.getPosition(), BoolOrOperator.INSTANCE, expression, parseLogicalXor());
        }
    ...
    private IExpression parseLogicalXor() throws ExpressionException {
        ...
        while (lexer.isThenNextAndNotEnd(Id.XOR)) {
            expression = new Call(lexer.getPosition(), BoolXorOperator.INSTANCE, expression, parseLogicalAnd());
        }
    ...
    private IExpression parseLogicalAnd() throws ExpressionException {
        ...
        while (lexer.isThenNextAndNotEnd(Id.AND)) {
            expression = new Call(lexer.getPosition(), BoolAndOperator.INSTANCE, expression, parseLogicalNot());
        }
    ...
    private IExpression parseConditional() throws ExpressionException {
        ...
        if (lexer.isThenNext(Id.IS)) {
            boolean not = false;
            int start = lexer.getPosition();

            if (lexer.isThenNext(Id.NOT)) {
                not = true;
            }
            ...
            return switch (lexer.next().id()) {
                ...
                case DISTINCT -> {
                    if (lexer.isThenNext(Token.Id.FROM)) {
    ...
    private IExpression parseRelational() throws ExpressionException {
        ...
        boolean not = lexer.isThenNext(Id.NOT);

        int start = lexer.getPosition();

        if (lexer.isThenNext(Id.LIKE)) {
            IExpression pattern = this.parseAdditive();

            if (lexer.isThenNext(Id.ESCAPE)) {
            ...
            return expression;
        } else if (lexer.isThenNext(Id.ILIKE)) {
            IExpression pattern = this.parseAdditive();
            if (lexer.isThenNext(Id.ESCAPE)) {
            ...
            return expression;
        } else if (lexer.isThenNext(Id.IN)) {
            ...
            return new Call(
                    ...
                    expression,
                    new Array(list));
        } else if (lexer.isThenNext(Id.BETWEEN)) {
            Operator operator;
            if (lexer.isThenNext(Id.ASYMMETRIC)) {
                operator = BetweenAsymmetricOperator.INSTANCE;
            } else if (lexer.isThenNext(Id.SYMMETRIC)) {
            ...
            return expression;
        } else if (lexer.isThenNext(Id.SIMILAR)) {
    ...
    private IExpression parsePrimary() throws ExpressionException {
        ...
        if (lexer.isThenNext(Id.LBRACKET)) {
    ...
    private IExpression parseTerm() throws ExpressionException {
        ...
        switch (token.id()) {
            ...
            case LPARENTHESIS:
                if (lexer.hasNext()) {
                    IExpression expression = this.parseLogicalOr();
                    if (lexer.isThenNext(Id.RPARENTHESIS)) {
                        return expression;
                    }
                }
    ...
    private IExpression parseComparison() throws ExpressionException {
        ...
        if (lexer.isThenNextAndNotEnd(Id.GTE)) {
            return new Call(start, GreaterThanOrEqualOperator.INSTANCE, expression, this.parseRelational());
        }
    ...
    private IExpression parseArray(final Token token) throws ExpressionException {
        ...
        // Empty array
        if (lexer.isThenNext(Id.RBRACKET)) {
        ...
        } while (lexer.isThenNext(Id.COMMA));
    ...
    private IExpression parseCase() throws ExpressionException {
        ...
        while (lexer.isThenNext(Id.WHEN)) {
            IExpression expression = this.parseLogicalOr();

            // Simple case with multi values
            if (simple && lexer.isThenNext(Id.COMMA)) {
    ...
    private boolean parseNullTreatment(Function function) throws ExpressionException {
        if (lexer.isThenNext(Id.IGNORE)) {
            if (lexer.isThenNext(Id.NULLS)) {
                return true;
            }
            throw new ExpressionParseException(lexer.getPosition(), ErrorCode.SYNTAX_ERROR_FUNCTION, function);
        }
        if (lexer.isThenNext(Id.RESPECT)) {
            if (lexer.isThenNext(Id.NULLS)) {
                return false;
            }
            throw new ExpressionParseException(lexer.getPosition(), ErrorCode.SYNTAX_ERROR_FUNCTION, function);
    ...
    private IExpression parseFunctionCount(Token token, Function function)
        ...
        // COUNT(*) no operand
        if (lexer.isThenNext(Id.STAR)) {
            aggregator = CountFunction.COUNT_ALL;
            // Use fictive operand
            operands.add(Literal.NULL);
        } else if (lexer.isThenNext(Id.DISTINCT)) {
    ...
    private IExpression parseFunctionListAgg(Token token, Function function)
        ...
        if (lexer.isThenNext(Id.DISTINCT)) {
    ...
    private IExpression parseFunctionJsonValue(Token token, Function function)
        ...
        operands.add(this.parseLogicalOr());
        if (lexer.isThenNext(Id.COMMA)) {
            operands.add(this.parseLogicalOr());
            if (lexer.isThenNext(Id.RETURNING)) {
                operands.add(this.parseLiteralType(lexer.next()));
            }
        }
    ...
    private IExpression parseLiteralType(Token token) throws ExpressionException {
        ...
        if (lexer.isThenNext(Id.LPARENTHESIS)) {
