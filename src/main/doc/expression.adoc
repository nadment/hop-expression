...
Interval literals defines a constant time period. You can use an interval to add or subtract a period of time to/from a date or timestamp.
...
|right|xref:plus.adoc["+",role=op], xref:negative.adoc["-",role=operator]|Positive, Negative
