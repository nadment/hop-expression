...
public class ValueInteger extends Value {
  ...
  protected ValueInteger(long value) {
  ...
  public int hashCode() {
    /*
     * NaNs are normalized in Value.of() method, so it's safe to use doubleToRawLongBits() instead
     * of doubleToLongBits() here.
     */
  ...
  public Value add(Value v) {
    ...
    if (this.getType().compareTo(v.getType()) >= 0) {
      return new ValueInteger(value + v.toInteger());
    }
  ...
  public Value multiply(Value v) {
    ...
    return ValueNumber.of(this.toNumber() * v.toNumber());
  }
  ...
  public Value divide(Value v) {
    ...
    if (v.isBigNumber()) {
      return ValueBigNumber.of(this.toBigNumber().divide(v.toBigNumber(),
          ValueBigNumber.DEFAULT_SCALE, BigDecimal.ROUND_HALF_UP));
    }
