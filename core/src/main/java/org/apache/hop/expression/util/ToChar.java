...
public class ToChar {
   ...
   *
   * @param value value
   * @param tzd if {@code true} return TZD (time zone region with Daylight Saving Time information
   *     included), if {@code false} return TZR (time zone region)
  ...
  //            return tz.getID();
  //        }
  //        return DateTimeUtils.timeZoneNameFromOffsetMins(((ValueTimestampTimeZone)
  ...
  public static String toChar(ZonedDateTime value, String format, Locale local) {
    ...
    for (int index = 0, length = format.length(); index < length; ) {
      ...
      // 4-digit year
      else if (containsAt(format, index, "YYYY") != null
          || containsAt(format, index, "RRRR") != null) {
        int year = Math.abs(value.getYear());
        if (fillMode) {
