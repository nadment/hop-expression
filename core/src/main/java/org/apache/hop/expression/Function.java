    ...
    try {
      switch (kind) {
        ...
        case ATAN2:
          {
            ...
            if (x.isNull() || y.isNull()) return Value.NULL;
        ...
        case BITGET:
          {
            Value v0 = args[0].eval(context);
            if (v0.isNull()) return v0;
            Value v1 = args[1].eval(context);
            if (v1.isNull()) return v1;
        ...
        case CONTAINS:
          {
            ...
            if (left.isNull() && right.isNull()) return Value.TRUE;

            if (left.isNull() || right.isNull()) return Value.FALSE;

            if (left.toString().contains(right.toString())) return Value.TRUE;
